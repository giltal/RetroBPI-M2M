...
echo "=== does the new S05powercap set the governor? ==="
...
echo "=== OLD defaults (schedutil, latency 128) vs NEW (performance, latency 256) ==="
for r in 1 2 3; do
  for g in /sys/devices/system/cpu/cpu*/cpufreq/scaling_governor; do echo schedutil > "$g" 2>/dev/null; done
  rm -f "$OVR"
  R=$(measure); printf 'round%s  OLD schedutil+128   xruns=%-4s nonRUN=%s\n' "$r" $R

  for g in /sys/devices/system/cpu/cpu*/cpufreq/scaling_governor; do echo performance > "$g" 2>/dev/null; done
  [ -f /tmp/ovr.keep ] && cp /tmp/ovr.keep "$OVR"
  R=$(measure); printf 'round%s  NEW performance+256 xruns=%-4s nonRUN=%s\n' "$r" $R
done

[ -f /tmp/ovr.keep ] && cp /tmp/ovr.keep "$OVR"
/etc/init.d/S05powercap start >/dev/null 2>&1
/etc/init.d/S12launcher start >/dev/null 2>&1
echo "### final state: governor=$(cat /sys/devices/system/cpu/cpu0/cpufreq/scaling_governor) override=$([ -f "$OVR" ] && echo present || echo MISSING)"
